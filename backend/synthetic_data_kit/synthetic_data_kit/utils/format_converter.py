...
def to_jsonl(data: List[Dict[str, Any]], output_path: str) -> str:
    ...
    # use ensure_ascii=False so non-ASCII characters are written as real Unicode
...
def to_chatml(qa_pairs: List[Dict[str, str]], output_path: str) -> str:
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        for pair in qa_pairs:
            ...
            f.write(json.dumps({"messages": chat}, ensure_ascii=False) + '\n')
...
def to_hf_dataset(qa_pairs: List[Dict[str, str]], output_path: str) -> str:
    ...
    if not qa_pairs:
        raise ValueError(
            "No QA pairs found to convert into a HuggingFace dataset. "
            "Ensure the pipeline generated QA pairs before exporting."
        )
